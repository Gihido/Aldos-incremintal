...
local function getServerMaxActiveCoins()
	...
	if value <= 0 then
		warn("[CoinService] UpgradeService.GetServerMaxActiveCoins returned", value, "; using minimum 1")
	end
...
local function getCoinsPerPickup(player)
	if type(UpgradeService.GetCoinsPerPickup) ~= "function" then
		warn("[CoinService] UpgradeService.GetCoinsPerPickup is not available; using fallback pickup amount")
		return 1
	end
...
local function awardCoinsToPlayer(player, amount)
	if type(DataService.AddCoins) ~= "function" then
		warn("[CoinService] DataService.AddCoins is not available; coin award skipped")
		return nil
	end
...
local function finishCoinCollection(player, coin, amount)
	if not player or not coin or not coin.Parent then
		return
	end
	...
	if coins and typeof(newCoinBalance) == "number" then
				...
				if isInside then
					for coin in pairs(activeCoins) do
						if coin
							...
							and isCoinInsideCollectSquare(player, coin)
						then
...
function CoinService.Init(remotes)
	...
	if typeof(remotes) == "Instance" then
		coinCollectedEffect =
			remotes:FindFirstChild("CoinCollectedEffect")
			or (remotesFolder and remotesFolder:FindFirstChild("CoinCollectedEffect"))

		collectZoneStateRemote =
			remotes:FindFirstChild("CollectZoneState")
			or (remotesFolder and remotesFolder:FindFirstChild("CollectZoneState"))
	elseif type(remotes) == "table" then
		coinCollectedEffect =
			remotes.CoinCollectedEffect
			or (remotesFolder and remotesFolder:FindFirstChild("CoinCollectedEffect"))

		collectZoneStateRemote =
			remotes.CollectZoneState
			or (remotesFolder and remotesFolder:FindFirstChild("CollectZoneState"))
	else
		coinCollectedEffect = remotesFolder and remotesFolder:FindFirstChild("CoinCollectedEffect")
		collectZoneStateRemote = remotesFolder and remotesFolder:FindFirstChild("CollectZoneState")
	end

	print("[CoinService] coinTemplate:", coinTemplate)
	print("[CoinService] zonePart:", zonePart)
	print("[CoinService] coinCollectedEffect:", coinCollectedEffect)
	print("[CoinService] collectZoneStateRemote:", collectZoneStateRemote)
